...
def recon_loss(config, real_image, fake_image, mask_1, gauss_kernel, az_loss):
    ...
    w_az_loss_high = config.w_az_loss_high
    ...
    if w_recon_fft_az > 0:
        azimuthal_real = azimuthal(config, real_image, mask_1)
        azimuthal_fake = azimuthal(config, fake_image, mask_1)
        loss_rec_fft_az = az_loss(azimuthal_real.detach(), azimuthal_fake.detach())
        #loss_rec_fft_az = az_loss(azimuthal_real[:,:,:config.gauss_kernel_size].detach(), azimuthal_fake[:,:,:config.gauss_kernel_size].detach())
        #loss_rec_fft_az += az_loss(azimuthal_real[:,:,config.gauss_kernel_size:].detach(), azimuthal_fake[:,:,config.gauss_kernel_size:].detach()) * w_az_loss_high
...
def trans_loss(config, real_image, fake_image, mask_h, gauss_kernel, az_loss):
    ...
    if w_trans_fft_az > 0:
        azimuthal_real = azimuthal(config, real_image, mask_h)
        azimuthal_fake = azimuthal(config, fake_image, mask_h)
        loss_trans_fft_az = az_loss(azimuthal_real.detach(), azimuthal_fake.detach())
        #loss_trans_fft_az = az_loss(azimuthal_real[:, :, :config.gauss_kernel_size].detach(), azimuthal_fake[:, :, :config.gauss_kernel_size].detach())
        #loss_trans_fft_az += az_loss(azimuthal_real[:, :, config.gauss_kernel_size:].detach(), azimuthal_fake[:, :, config.gauss_kernel_size:].detach()) * w_az_loss_high
        loss_trans_fft_az = w_trans_fft_az * loss_trans_fft_az
    else:
        loss_trans_fft_az = 0

    ## Total translation loss
    trans_loss = loss_trans_pix_l1 + loss_trans_pix_moment + loss_trans_fft_l1 + loss_trans_fft_az
    return trans_loss

def PD_moment(config, target_feat_list, mix_feat_list):
    w_trans_PD_moment = config.w_trans_PD_moment
    cmd_weights = config.CMDweights

    loss_trans_PD_moment = 0
    if w_trans_PD_moment > 0:
        """for i in range(len(target_feat_list)):
            real_embedding = target_feat_list[i]
            fake_embedding = mix_feat_list[i]

            trans_PD_moment = CMD_loss(torch.sigmoid(real_embedding), k=len(cmd_weights), weights=cmd_weights)
            loss_trans_PD_moment += trans_PD_moment(torch.sigmoid(fake_embedding))
        loss_trans_PD_moment = w_trans_PD_moment * loss_trans_PD_moment / len(target_feat_list)
        """
        real_embedding = target_feat_list[-2]
        fake_embedding = mix_feat_list[-2]

        trans_PD_moment = CMD_loss(torch.sigmoid(real_embedding), k=len(cmd_weights), weights=cmd_weights)
        loss_trans_PD_moment = trans_PD_moment(torch.sigmoid(fake_embedding)) * w_trans_PD_moment

    return loss_trans_PD_moment
